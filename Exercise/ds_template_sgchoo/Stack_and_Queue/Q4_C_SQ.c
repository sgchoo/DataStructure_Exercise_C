...
{
	/* add your code here */
	Stack tempStack;								// reverse하기 위한 Stack 선언
	int length = q->ll.size;

	for(int i = 0; i < length; i++)
	{
		int tempItem = dequeue(q);
		push(&tempStack, tempItem);
	}

	int stackLength = (&tempStack)->ll.size;		// 임시 스택 길이
	for(int i = 0; i < stackLength; i++)
	{
		int reversedItem = pop(&tempStack);
		enqueue(q, reversedItem);
	}
}
