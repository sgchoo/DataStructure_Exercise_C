...
void removeUntil(Stack *s, int value)
{
	...
	for(int i = 0; i < stackSize; i++)				// value가 나올때까지 tempStack에 저장한다.
	{
		int tempItem = pop(s);
	...
	removeAllItemsFromStack(s);						// 남아있는 원소를 지운다.

	int tempStackSize = (&tempStack)->ll.size;
	for(int i = 0; i < tempStackSize; i++)
	{
		int remainItem = pop(&tempStack);
		push(s, remainItem);
	}
}
