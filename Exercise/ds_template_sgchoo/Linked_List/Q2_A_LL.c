...
{
    /* add your code here */
	int length = ll1->size < ll2->size ? ll1->size : ll2->size;

	ListNode *list_1 = ll1->head;
	ListNode *preNode;																// 이전 노드 저장용

	for(int i = 0; i < length; i++)
	{
		ListNode *deleteNode = ll2->head;
		ListNode *newNode = (ListNode *)malloc(sizeof(ListNode));
		newNode->item = ll2->head->item;
		newNode->next = list_1->next;
