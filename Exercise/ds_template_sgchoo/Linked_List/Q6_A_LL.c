...
int moveMaxToFront(ListNode **ptrHead)
{
	...
	ListNode *maxItemNode = *ptrHead;
	ListNode *tempPreNode;
	ListNode *preNode;
	ListNode *newHeadNode = (ListNode *)malloc(sizeof(ListNode));

	while(tempHead != NULL)
	{
		if(tempHead->item > maxItemNode->item)
		{
			preNode = tempPreNode;
			maxItemNode = tempHead;
		}
	...
	newHeadNode->item = maxItemNode->item;
	newHeadNode->next = *ptrHead;

	*ptrHead = newHeadNode;
	preNode->next = maxItemNode->next;
	free(maxItemNode);

	return maxItemNode->item;
}
