...
int hasGreatGrandchild(BTNode *node)
{
	/* add your code here */
    // solution
    int leftSubTree, rightSubTree, depth;

    if(node == NULL)    return -1;
    else
    {
        leftSubTree = hasGreatGrandchild(node->left);
        rightSubTree = hasGreatGrandchild(node->right);
        if(leftSubTree > rightSubTree)  depth = leftSubTree;
        else                            depth = rightSubTree;
        depth += 1;
        if(depth >= 3)  printf("%d\n", node->item);
        return depth;
    }
